...
- [ ] **Tests Pass:** The pytest suite passes under strict settings (`-W error`), including property-based tests where present.
- [ ] **Coverage:** Test coverage meets or exceeds **>90%** on new files.
- [ ] **Linting:** `ruff check` passes without warnings.
- [ ] **Typing:** `basedpyright` type check completes with zero errors.
